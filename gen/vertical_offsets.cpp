...
// Generated on 2019-11-05  23:59:47
...
class test {
 public:
  ...
  struct stencil_17 {
    // Intervals
    using interval_start__start_ = gridtools::interval<gridtools::level<0, 1, 3>, gridtools::level<0, 1, 3>>;
    using interval_start_plus_1_end_ = gridtools::interval<gridtools::level<0, 2, 3>, gridtools::level<1, -1, 3>>;
    using axis_stencil_17 = gridtools::interval<gridtools::level<0, -1, 3>, gridtools::level<1, 1, 3>>;
    ...
    struct stage_0_0 {
      ...
      GT_FUNCTION static void apply(Evaluation& eval, interval_start__start_) {
        eval(out_field(0, 0, 0)) = eval(in_field(0, 0, 0));
      }

      template <typename Evaluation>
      GT_FUNCTION static void apply(Evaluation& eval, interval_start_plus_1_end_) {}
    };

    struct stage_1_0 {
      using in_field = gridtools::accessor<0, gridtools::intent::in, gridtools::extent<0, 0, 0, 0, -1, -1>>;
      using out_field = gridtools::accessor<1, gridtools::intent::inout, gridtools::extent<0, 0, 0, 0, 0, 0>>;
      using param_list = gridtools::make_param_list<in_field, out_field>;
    ...
    stencil_17(const gridtools::clang::domain& dom) {
      ...
      gridtools::halo_descriptor dj = {dom.jminus(), dom.jminus(), dom.jplus(), dom.jsize() - 1 - dom.jplus(),
                                       dom.jsize()};
      grid_stencil_17 grid_(di, dj, {dom.kminus(), dom.ksize() == 0 ? 0 : dom.ksize() - dom.kplus()});
      ...
      m_stencil = gridtools::make_computation<backend_t>(
          ...
          gridtools::make_multistage(
              ...
              gridtools::make_stage_with_extent<stage_0_0, gridtools::extent<0, 0, 0, 0>>(p_in_field(), p_out_field())),
 ...
 public:
  ...
  void run(storage_ijk_t in_field, storage_ijk_t out_field) {
    ...
    m_stencil_17.get_stencil()->run(p_in_field{} = in_field, p_out_field{} = out_field);
    sync_storages(in_field, out_field);
  }
  ...
  double get_total_time() {
    double res = 0;
    res += m_stencil_17.get_stencil()->get_time();
    return res;
  }
};
}  // namespace gt
