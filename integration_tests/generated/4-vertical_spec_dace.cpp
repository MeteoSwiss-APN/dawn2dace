        ...
        for (auto k = 0; k < K; k += 1) {
            for (auto i = halo_size; i < (I - halo_size); i += 1) {
                for (auto j = halo_size; j < (J - halo_size); j += 1) {
                    {
                        auto __data_in_2_input = dace::ArrayViewIn<double, 0, 1, 1> (data_in_2_t + ((((I * j) * (K + 1)) + (I * k)) + i));
                        dace::vec<double, 1> data_in_2_input = __data_in_2_input.val<1>();

                        auto __data_out = dace::ArrayViewOut<double, 0, 1, 1> (data_out_t + ((((I * j) * (K + 1)) + (I * k)) + i));
    ...
    __state_IIRToSDFG_state_1:
    {
        ...
        for (auto k = 3; k < K; k += 1) {
            for (auto i = halo_size; i < (I - halo_size); i += 1) {
                for (auto j = halo_size; j < (J - halo_size); j += 1) {
                    {
                        auto __data_in_1_input = dace::ArrayViewIn<double, 0, 1, 1> (data_in_1_t + ((((I * j) * (K + 1)) + (I * k)) + i));
            ...
            // statement_map[k=3:K, i=halo_size:I - halo_size, j=halo_size:J - halo_size]
        }
    }
