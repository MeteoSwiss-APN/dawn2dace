...
void __program_IIRToSDFG_internal(double * __restrict__ in_field_t, double * __restrict__ out_field_t, int I, int J, int K, int halo_size)
{
    ...
    {
        double *__local_ee_30_t = new double DACE_ALIGN(64)[J * K * I];
                ...
                for (auto j = halo_size; j < (J - halo_size); j += 1) {
                    {
                        ...
                        auto ____local_ee_30 = dace::ArrayViewOut<double, 0, 1, 1> (__local_ee_30_t + ((((I * K) * j) + (I * k)) + i));
                        dace::vec<double, 1> __local_ee_30;

                        ///////////////////
                        // Tasklet code (statement)
                        __local_ee_30 = (in_field + 5);
                        ///////////////////
        ...
        for (auto i = halo_size; i < (I - halo_size); i += 1) {
            for (auto k = 0; k < K; k += 1) {
                for (auto j = halo_size; j < (J - halo_size); j += 1) {
                    {
                        auto ____local_ee_30 = dace::ArrayViewIn<double, 0, 1, 1> (__local_ee_30_t + ((((I * K) * j) + (I * k)) + i));
                        dace::vec<double, 1> __local_ee_30 = ____local_ee_30.val<1>();
