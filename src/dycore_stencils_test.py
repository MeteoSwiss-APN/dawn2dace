...
class Transcompiler():
    ...
    def test6_transforms_to_gpu(self):
        sdfg = SDFG.from_file("gen/DyCore/LibraryNodes/" + self.file_name + ".sdfg")

        canonicalize_sdfg(sdfg)
        sdfg.save("gen/" + self.file_name + "_canonicalize.sdfg")
        
        sdfg.apply_transformations_repeated([StateFusion, InlineSDFG, StencilFusion])
        sdfg.save("gen/" + self.file_name + "_fused.sdfg")
        sdfg = SDFG.from_file("gen/" + self.file_name + "_fused.sdfg")
        sdfg.expand_library_nodes()
        sdfg.save("gen/" + self.file_name + "_expanded.sdfg")
        sdfg = SDFG.from_file("gen/" + self.file_name + "_expanded.sdfg")
        sdfg.apply_transformations(GPUTransformSDFG, validate=False)
        sdfg.validate()
        sdfg.save("gen/DyCore/GPU/" + self.file_name + ".sdfg")
